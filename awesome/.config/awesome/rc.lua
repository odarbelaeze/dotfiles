...
root.buttons(gears.table.join(awful.button({}, 3,
                                           function() MY_MAIN_MENU:toggle() end),
...
awful.key({MODKEY}, "/", function() awful.util.spawn("rofi -show combi") end,
...
CLIENT_KEYS = gears.table.join(awful.key({MODKEY, "Control"}, "i", function(c)
...
for i, _tag in ipairs(TAGS) do
    ...
    -- Toggle tag display.
                                   awful.key({MODKEY, "Shift"}, _tag,
                                             function()
        local screen = awful.screen.focused()
        local tag = screen.tags[i]
        if tag then awful.tag.viewtoggle(tag) end
    end, {description = "toggle tag " .. _tag, group = "tag"}),
    -- Move client to tag.
                                   awful.key({MODKEY, "Control"}, _tag,
                                             function()
        if client.focus then
            local tag = client.focus.screen.tags[i]
            if tag then client.focus:move_to_tag(tag) end
        end
    end, {description = "move focused client to tag " .. _tag, group = "tag"}) -- Toggle tag on focused client.
    -- awful.key({ MODKEY, "Control", "Shift" }, _tag,
    --           function ()
    --               if client.focus then
    --                   local tag = client.focus.screen.tags[i]
    --                   if tag then
    ...
    --               end
    --           end,
    --           {description = "toggle focused client on tag " .. _tag, group = "tag"})
    )
end

CLIENT_BUTTONS = gears.table.join(awful.button({}, 1, function(c)
...
awful.rules.rules = {
    ...
    }, -- Set Jack to always map on the tag named "Jack" on screen 1.
    {
