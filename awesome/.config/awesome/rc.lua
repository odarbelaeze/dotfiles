...
GLOBAL_KEYS = gears.table.join(
    ...
    end, { description = "swap with client down", group = "client" }),
    awful.key({ MODKEY, "Control" }, "k", function()
        awful.client.swap.bydirection("up")
    end, { description = "swap with client up", group = "client" }),
    ...
    end, { description = "increase number of stacks", group = "layout" }),
    awful.key({ MODKEY, "Shift", "Control" }, "j", function()
        awful.tag.incncol(-1, nil, true)
    end, { description = "decrease number of stacks", group = "layout" }),
    ...
    awful.key({ MODKEY, "Control" }, "p", function()
        awful.util.spawn_with_shell("flameshot full --clipboard")
    ...
    end, { description = "toggle play music", group = "media" }),
...
CLIENT_KEYS = gears.table.join(
    ...
    end, { description = "restore maximized", group = "client" }),
    awful.key(
        { MODKEY, "Control" },
        "space",
...
for i, _tag in ipairs(TAGS) do
    GLOBAL_KEYS = gears.table.join(
        GLOBAL_KEYS,
        -- View tag only
        awful.key({ MODKEY }, _tag, function()
            local screen = awful.screen.focused()
            local tag = screen.tags[i]
            if tag then
                tag:view_only()
            end
        end, { description = "view tag " .. _tag, group = "tag" }),
        -- Move client to tag.
        awful.key({ MODKEY, "Control" }, _tag, function()
            if client.focus then
        ...
        end, { description = "move focused client to tag " .. _tag, group = "tag" }),
        -- Toggle client to tag.
        awful.key({ MODKEY, "Shift" }, _tag, function()
            if client.focus then
                local tag = client.focus.screen.tags[i]
                if tag then
...
client.connect_signal("unmanage", function(c)
    -- Check if the closed client was focused
    if client.focus == c or not client.focus then
        -- Find the next available client
        local clients = awful.client.visible(c.screen, c.tag)
        if #clients > 0 then
            local next_client = clients[#clients]
            next_client:emit_signal("request::activate", "unmanage", { raise = true })
        end
    end
end)
...
client.connect_signal("property::urgent", function(c)
    if c.urgent then
        -- If the client is urgent, show a notification
...
tag.connect_signal("property::selected", function()
    focus_on_last_in_history(mouse.screen)
end)

-- Autostart
awful.spawn.with_shell("picom")
awful.spawn.with_shell("polkit")
awful.spawn.with_shell("nitrogen --restore")
